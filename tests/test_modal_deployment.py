...
def test_build_feishu_internal_plan_marks_plain_text_as_external_exec_candidate():
    ...
    assert result["provider_plan"]["model"] == "openai/gpt-oss-20b:free"
    assert result["provider_plan"]["session_model"] == "openrouter/free"
    assert result["provider_plan"]["request_timeout_ms"] == 4500
    assert result["provider_plan"]["max_attempts"] == 1
    assert result["provider_plan"]["cache_mode"] == "skip"
    assert result["provider_plan"]["cache_scope"] == "user"
