...
WEB_APP_MEMORY_SNAPSHOT_ENABLED = str(
    os.getenv("HERMES_MODAL_WEB_APP_MEMORY_SNAPSHOT_ENABLED", "true")
).strip().lower() not in {"0", "false", "no", "off", ""}
FEISHU_INGRESS_MEMORY_SNAPSHOT_ENABLED = str(
    os.getenv("HERMES_MODAL_FEISHU_INGRESS_MEMORY_SNAPSHOT_ENABLED", "true")
).strip().lower() not in {"0", "false", "no", "off", ""}
FEISHU_ACK_REACTION_MEMORY_SNAPSHOT_ENABLED = str(
    os.getenv("HERMES_MODAL_FEISHU_ACK_REACTION_MEMORY_SNAPSHOT_ENABLED", "true")
).strip().lower() not in {"0", "false", "no", "off", ""}
CHAT_QUEUE_MEMORY_SNAPSHOT_ENABLED = str(
    os.getenv("HERMES_MODAL_CHAT_QUEUE_MEMORY_SNAPSHOT_ENABLED", "true")
).strip().lower() not in {"0", "false", "no", "off", ""}
...
async def _send_feishu_local_registry_menu_card(payload: Mapping[str, Any]) -> bool:
    ...
    provider_slug, view_name = _FEISHU_LOCAL_MODEL_MENU_MAP.get(event_key, (None, None))
    if not provider_slug or not view_name:
        return False
    ...
    card = _build_local_registry_provider_card(provider_slug=provider_slug, view_name=view_name)
...
def _add_feishu_ack_reaction_from_payload(
    ...
    *,
    ...
    from_request_elapsed_ms = (
        max(0, int(time.time() * 1000) - int(request_started_at_ms))
...
async def _spawn_feishu_ack_reaction_async(
    *,
    ...
    request_started_at_ms = int(request_started_at * 1000) if request_started_at is not None else None
    try:
...
def _validate_feishu_message_ingress_impl(*, message_text: str = "selftest ingress path") -> dict[str, Any]:
    import httpx
    ...
    payload = {
        ...
        "event": {
            "sender": {
                "sender_id": {"open_id": "ou_selftest_ingress"},
                "sender_type": "user",
            },
            "message": {
                "message_id": message_id,
                "chat_id": "oc_selftest_ingress",
                "chat_type": "p2p",
                "message_type": "text",
...
async def _spawn_feishu_message_inline_async(
    *,
    ...
    spawn_handle = getattr(worker, "spawn")
    if hasattr(spawn_handle, "aio"):
        await spawn_handle.aio(payload=payload)  # type: ignore[union-attr]
    else:
        await asyncio.to_thread(spawn_handle, payload=payload)
    return {
        "status": "scheduled",
...
if modal is not None:
    ...
    class FeishuInlineMessageWorker:
        ...
        def handle(self, payload: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                asyncio.run(_dispatch_feishu_payload(payload, await_background_tasks=True))
            finally:
            ...
            logger.warning(
                "[Feishu] inline worker done event_id=%s partition=%s lane=%s worker_boot_id=%s reused=%s inline_elapsed_ms=%s",
    ...
    def validate_feishu_message_ingress(message_text: str = "selftest ingress path") -> dict[str, Any]:
        return _validate_feishu_message_ingress_impl(message_text=message_text)
