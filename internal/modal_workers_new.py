...
_process_feishu_ack_reaction_deps = {}
_process_feishu_event_deps = {}
_process_feishu_message_inline_deps = {}
_process_feishu_background_exec_deps = {}
_process_chat_queue_deps = {}

def _set_ack_reaction_deps(deps: dict[str, Any]) -> None:
    global _process_feishu_ack_reaction_deps
    _process_feishu_ack_reaction_deps = deps

def _set_event_deps(deps: dict[str, Any]) -> None:
    global _process_feishu_event_deps
    _process_feishu_event_deps = deps

def _set_message_inline_deps(deps: dict[str, Any]) -> None:
    global _process_feishu_message_inline_deps
    _process_feishu_message_inline_deps = deps

def _set_background_exec_deps(deps: dict[str, Any]) -> None:
    global _process_feishu_background_exec_deps
    _process_feishu_background_exec_deps = deps

def _set_chat_queue_deps(deps: dict[str, Any]) -> None:
    global _process_chat_queue_deps
    _process_chat_queue_deps = deps
...
def register_modal_workers(
    *,
    ...
    _set_ack_reaction_deps(deps)
    _set_event_deps(deps)
    _set_message_inline_deps(deps)
    _set_background_exec_deps(deps)
    _set_chat_queue_deps(deps)
    ...
    def process_feishu_ack_reaction(
        ...
        deps = _process_feishu_ack_reaction_deps
        snapshot_context = {}
        if deps["FEISHU_ACK_REACTION_MEMORY_SNAPSHOT_ENABLED"]:
    ...
    def process_feishu_event(
        ...
        deps = _process_feishu_event_deps
        snapshot_context = {}
        if deps["FEISHU_INGRESS_MEMORY_SNAPSHOT_ENABLED"]:
            snapshot_context = deps["prepare_worker_snapshot_context"](
        ...
        return deps["spawn_chat_queue_worker_sync"](
            ...
            inline_budget_ms=deps["DEFAULT_FEISHU_INLINE_WORKER_HARD_BUDGET_MS"],
            logger=deps["logger"],
        )
    ...
    def process_feishu_message_inline(
        ...
        deps = _process_feishu_message_inline_deps
        if warmup_only:
            return {"status": "warmup"}
        snapshot_context = {}
        if deps["FEISHU_CHAT_MEMORY_SNAPSHOT_ENABLED"]:
            snapshot_context = deps["prepare_worker_snapshot_context"](
    ...
    def process_feishu_background_exec(
        ...
        deps = _process_feishu_background_exec_deps
        if warmup_only:
            return {"status": "warmup"}
        snapshot_context = {}
        if deps["FEISHU_BACKGROUND_EXEC_MEMORY_SNAPSHOT_ENABLED"]:
            snapshot_context = deps["prepare_worker_snapshot_context"](
