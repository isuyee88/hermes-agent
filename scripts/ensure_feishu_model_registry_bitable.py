...
def main() -> int:
    ...
    parser.add_argument("--app-token", default="", help="Feishu Bitable app token. Falls back to FEISHU_BITABLE_APP_TOKEN.")
    parser.add_argument("--table-id", default="", help="Existing table id. If omitted, the script finds or creates by table name.")
    ...
    app_token = str(args.app_token or os.getenv("FEISHU_BITABLE_APP_TOKEN") or "").strip()
    if not app_token:
        print(json.dumps({"status": "error", "error": "FEISHU_BITABLE_APP_TOKEN is required"}, ensure_ascii=False, indent=2))
        return 1
